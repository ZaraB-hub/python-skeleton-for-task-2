...
def add_peer(peer):
    ...
    try:
        ip = ipaddress.ip_address(peer.host) #converts to an ip adress object
        if ip.is_loopback:
            return
        if ip.is_multicast:
            return
...
def mk_peers_msg():
    pl = [f"{const.ADDRESS}:{const.PORT}"]
    other_peers = [f'{peer}' for peer in PEERS if f'{peer}' != pl[0]]
    if len(other_peers) > 29:
        other_peers = random.sample(other_peers, 29)
    pl.extend(other_peers)
...
def validate_hostname(host_str):
    ...
    # Check 1: Allowed characters and length
    if not re.match(pattern, host_str):
        return False
    
    # Check 2: Ensure there’s at least one dot not at the beginning or end
    if "." not in host_str or host_str.startswith('.') or host_str.endswith('.'):
        return False
    
    # Check 3: Ensure there’s at least one letter in the hostname
    if not any(c.isalpha() for c in host_str):
        return False
    
    # If all checks pass, it's a valid hostname
    return True

# returns true iff host_str is a valid ipv4 address
def validate_ipv4addr(host_str):
    try:
        # Try to create an IPv4 address object; if it fails, it's not valid
...
def validate_peer_str(peer_str):
    # Split peer string into host and port
    if ':' not in peer_str:
        raise ErrorInvalidFormat("Invalid peer format: missing port separator")
    ...
    try:
        port = int(port_str)
        if not (1 <= port <= 65535):
            raise ErrorInvalidFormat("Invalid port range")
    except ValueError:
        raise ErrorInvalidFormat("Port must be an integer")
    
    # First, check if it's a valid IPv4 address
    if validate_ipv4addr(host):
        return True
    
    # If not an IPv4 address, check if it's a valid hostname
    if validate_hostname(host):
        return True

    # If neither, raise an error
    raise ErrorInvalidFormat("Invalid host format")
...
def validate_ihaveobject_msg(msg_dict):
    if msg_dict.get('type') != 'ihaveobject':
        raise ErrorInvalidFormat("Message type is not 'ihaveobject'!")
    
    if 'objectid' not in msg_dict:
        raise ErrorInvalidFormat("Missing 'objectid' in ihaveobject message!")
    
    if not isinstance(msg_dict['objectid'], str):
        raise ErrorInvalidFormat("The 'objectid' must be a string!")
    
    if not re.fullmatch(r'[0-9a-f]{64}', msg_dict['objectid']):
        raise ErrorInvalidFormat("Invalid format for 'objectid'!")
    
# raise an exception if not valid
def validate_getobject_msg(msg_dict):
    if msg_dict.get('type') != 'getobject':
        raise ErrorInvalidFormat("Message type is not 'getobject'!")
    
    if 'objectid' not in msg_dict:
        raise ErrorInvalidFormat("Missing 'objectid' in getobject message!")
    
    if not isinstance(msg_dict['objectid'], str):
        raise ErrorInvalidFormat("The 'objectid' must be a string!")
    
    # Optionally validate the format of objectid
    if not re.fullmatch(r'[0-9a-f]{64}', msg_dict['objectid']):
        raise ErrorInvalidFormat("Invalid format for 'objectid'!")

# raise an exception if not valid
def validate_object_msg(msg_dict):
    if msg_dict.get('type') != 'object':
        raise ErrorInvalidFormat("Message type is not 'object'!")
    
    if 'object' not in msg_dict:
        raise ErrorInvalidFormat("Missing 'object' key in object message!")
    
    if not isinstance(msg_dict['object'], dict):
        raise ErrorInvalidFormat("The 'object' must be a dictionary!")
    
    # Additional validation can be done based on the structure of the object (e.g., block or transaction)
    if 'type' not in msg_dict['object']:
        raise ErrorInvalidFormat("The object data must contain a 'type' field!")
    
    obj_type = msg_dict['object']['type']
    if obj_type not in ['transaction', 'block']:
        raise ErrorInvalidFormat("The object type must be either 'transaction' or 'block'!")
...
from objects import get_objid

def store_object(obj_dict):
    objid = get_objid(obj_dict)  
    obj_type = obj_dict.get('type', 'unknown')
    obj_data = json.dumps(obj_dict)
    
    with sqlite3.connect(DB_NAME) as con:
        cur = con.cursor()
        cur.execute("INSERT OR REPLACE INTO objects VALUES (?, ?, ?)", (objid, obj_type, obj_data))
        con.commit()
...
def retrieve_object(objid):
    ...
    with sqlite3.connect(DB_NAME) as con:
        cur = con.cursor()
        cur.execute("SELECT object_data FROM objects WHERE object_id = ?", (objid,))
        result = cur.fetchone()
...
async def handle_ihaveobject_msg(msg_dict, writer):
    ...
    # Request the object if not in the database
    if not retrieve_object(objid):
...
async def handle_getobject_msg(msg_dict, writer):
    try:
        ...
        obj_data = retrieve_object(objid)  
        
        if obj_data:
            await write_msg(writer, mk_object_msg(obj_data))
            print(f"Sent object {objid} to requesting peer.")
        else:
            await write_msg(writer, mk_error_msg("Object not found", "UNKNOWN_OBJECT"))
            print(f"Object {objid} not found in database, sent UNKNOWN_OBJECT error.")
    
    except ErrorInvalidFormat as e:
        await write_msg(writer, mk_error_msg(str(e), "INVALID_FORMAT"))
        await write_msg(writer, mk_error_msg("UNKNOWN_OBJECT", "Object not found"))
...
async def gossip_object(objid):
   
    msg = mk_ihaveobject_msg(objid)
    
    for peer, writer in CONNECTIONS.items():
        try:
            await write_msg(writer, msg)
            print(f"Gossiped object {objid} to peer {peer}")
        except Exception as e:
            print(f"Failed to gossip to peer {peer}: {e}")
...
async def handle_object_msg(msg_dict, peer_self, writer):
    try:
        ...
        if retrieve_object(objid):
            return 
        
        store_object(obj_dict)
        
        # Gossip to peers that we now have this object
        await gossip_object(objid) 
    
    except ErrorInvalidFormat as e:
        # Send an error message back if the object message is invalid
        await write_msg(writer, mk_error_msg(str(e), "INVALID_FORMAT"))
...
async def handle_connection(reader, writer):
    ...
    try:
        ...
        print("Invalid format error: {}".format(e))
